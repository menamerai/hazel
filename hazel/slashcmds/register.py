...
class Register(app_commands.Group):
    ...
    async def hacker(self, interaction: discord.Interaction):
        ...
        if not check_root_leaf_branch(user_roles):
            ...
            await interaction.response.send_message(
                f"You do not have the required roles to register for the event. Please make sure you have exactly one root role, one leaf role, and one branch role. Modify your roles by reacting at #reaction-roles and try again, or contact an organizer.",
                ephemeral=True,
            )
            return
    ...
    @app_commands.describe(
        password="Password to register as a mentor, provided by an organizer"
    )
